...
//
//	ewdx_supio.cpp - supio implementation for the ewdx NDK port.
//
//	Derived from OpenHSP src/hsp3/linux/supio_linux.cpp (onitama/OpenHSP).
//	Adaptations:
//	  - includes ewdx_supio_decl.h (no platform macro; keeps HSPUTF8 off)
//	  - strcase/strstr2/strchr2/strchr3 are Shift-JIS aware (the game is
//	    SJIS; the Linux UTF-8 variants would corrupt trail bytes)
//	  - Alert/AlertV/Alertf go to logcat on device (stderr on host)
//	  - otherwise verbatim POSIX (gettime, dirlist, Replace*, Trim*, ...)
//
...
void CutLastChr( char *p, char code )
{
	...
	char *ss2;
	int i;
	(void)code;  // stock signature keeps the separator implicit (SJIS-aware below)
	ss = strchr2( p, '\\' );
	if ( ss != NULL ) {
...
//
//		ewdx log support (logcat on device, stderr on host)
//
void Alert( char *mes )
{
	EWDX_LOGE("%s", mes ? mes : "(null)");
}

void AlertV( char *mes, int val )
{
	EWDX_LOGE("%s%d", mes ? mes : "", val);
}
