...
static void *reffunc_intfunc( int *type_res, int arg )
{
	...
	case 0x188:								// logf
		dval = code_getd();
		/* ewdx: clamp non-positive inputs to 0.0 (was: raw log() -> -inf on
		   ARM64). The game's keta() digit counter is int(logf(v)/logf(10))+1;
		   with v==0 the -inf cast to int is INT_MIN on ARM64, so the score
		   HUD's "repeat keta(score)" runs ETERNAL (HSP maps any negative
		   repeat count to ETRLOOP), pow10(32) wraps to 0 in int32, and the
		   digit divide throws HSPERR_DIVIDED_BY_ZERO (#Error 19) the first
		   frame the score HUD draws with score==0 (v17 device log
		   ewdx-boot-234: 30+ identical digit-'0' draws, then Error 19).
		   Clamp to 1.0 => log(1)==0 => keta(0)==1 (score "0" renders as one
		   digit, PC behavior). All other logf call sites (seplay/set_volume
		   dB math) are guarded by >0 checks in the script, never see <=0. */
