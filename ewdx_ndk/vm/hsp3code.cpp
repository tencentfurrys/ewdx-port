...
/*------------------------------------------------------------*/
/*	ewdx: journal-on-overflow (v15 err7 forensics)
	Write var name + shape + index into ctx->refstr the moment an array
	index goes OOB; ewdx_boot_exec journals refstr when error 7 fires.
*/
/*------------------------------------------------------------*/
#include "hsp3debug.h"
void ewdx_vm_overflow_journal( PVal *pval, const char *where, int offset )
{
	HSPCTX *ctx;
	char *nm;
	int i, varid = -1;
	ctx = code_getctx();
	if ( ctx == NULL ) return;
	if ( pval != NULL && pval->flag != 0 ) {
		if ( ctx->mem_var != NULL ) {
			for ( i = 0; i < ctx->hsphed->max_val; i++ ) {
				if ( &(ctx->mem_var[i]) == pval ) { varid = i; break; }
			}
		}
		if ( varid >= 0 ) nm = code_getdebug_varname( varid );
		else nm = NULL;
		snprintf( ctx->refstr, HSPCTX_REFSTR_MAX - 1,
			"ERR7 %s var=%s id=%d flag=%d len=[%d,%d,%d,%d] idx=%d sup=%#x",
			( where != NULL ? where : "?" ),
			( nm != NULL ? nm : "??" ), varid, pval->flag,
			pval->len[1], pval->len[2], pval->len[3], pval->len[4],
			offset, (int)pval->support );
		ctx->refstr[ HSPCTX_REFSTR_MAX - 1 ] = 0;
	} else {
		snprintf( ctx->refstr, HSPCTX_REFSTR_MAX - 1, "ERR7 %s var=?(null pval) idx=%d",
			( where != NULL ? where : "?" ), offset );
	}
}
...
static inline void code_arrayint2( PVal *pval, int offset )
{
	...
	//
	if ( pval->arraycnt >= 5 ) {
		ewdx_vm_overflow_journal( pval, "dims>4", offset );
	...
	pval->arraycnt++;
	if ( offset < 0 ) {
		ewdx_vm_overflow_journal( pval, "neg idx", offset );
		throw HSPVAR_ERROR_ARRAYOVER;
	}
	if ( offset >= (pval->len[ pval->arraycnt ]) ) {
		...
		ewdx_vm_overflow_journal( pval, "range", offset );
		throw HSPVAR_ERROR_ARRAYOVER;
	}
	pval->offset += offset * pval->arraymul;
}
